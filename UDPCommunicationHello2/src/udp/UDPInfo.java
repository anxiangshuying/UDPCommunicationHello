...
class UDPInfo {
	...
	class SocketInfo {
		...
		int lP, rP, sbS, rbS, tO; // the local, remote port, sending/receiving
									// buffer size,timeout value
		boolean isIPV6, bounded, closed, connected, rU, bC;

		// is ipv6, soccket bounded, closed, connected, reuse and broadcasr
		// parametters
		SocketInfo() {
			clear();
		}
	...
	/**
	 * The main class to call, event is a string to characterize the call
	 * location in the code, s is the socket object.
	 */
	...
	private void print(String event) {
		if (sI.closed)
			...
			System.out.println(event + "\n" + "IPV6: " + sI.isIPV6 + "\n" + "local \tadress:" + sI.lA + "\t port:"
					+ sI.lP + "\n" + "remote \tadress:" + sI.rA + "\t port:" + sI.rP + "\n" + "bounded: " + sI.bounded
					+ "\n" + "closed: " + sI.closed + "\n" + "connected: " + sI.connected + "\n" + "timeout: " + sI.tO
					+ "\t broadcast: " + sI.bC + "\t reuse: " + sI.rU + "\t traffic:" + sI.tC + "\n" + "buffer \tsend:"
					+ sI.sbS + "\treceive:" + sI.rbS + "\n");
	}
}